...
import queue
import threading
from Views.Import.copySuccessView import CopySuccessView
...
class ScanSecureUsbView(tk.Frame):
    ...
    def copy_files_to_secure_usb(self):
        ...
        usb_mount_point = analyse.find_usb_mount_point()
        if usb_mount_point:
            for file_name in os.listdir(temp_dir):
                file_path = os.path.join(temp_dir, file_name)
                shutil.copy(file_path, usb_mount_point)
            shutil.rmtree(temp_dir)
            analyse.unmount_usb(usb_mount_point, self.message_queue)
